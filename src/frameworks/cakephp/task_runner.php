 ...
 * @license   Proprietary
 *
 * CakePHP 4/5 task runner. Boots the CakePHP application and executes
...
try {
    ...
    ddless_task_emit('error', ['message' => 'Failed to boot CakePHP: ' . $e->getMessage()]);
    ddless_task_done(false, $taskStartTime, 'Failed to boot CakePHP: ' . $e->getMessage());
    exit(1);
}

// Task Runner Command class
class DdlessCakePhpTaskRunnerCommand {
    private $__container;
...
// Execute user code
try {
    $command = new DdlessCakePhpTaskRunnerCommand();
    if ($container) {
    ...
    }, $command, DdlessCakePhpTaskRunnerCommand::class);
