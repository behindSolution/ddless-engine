 ...
 * @license   Proprietary
 *
 * CodeIgniter 4 method executor. Boots the CI4 application, resolves services,
 * and executes a single method/function with dependency injection support.
 */
...
function ddless_serialize_value($value, int $depth = 0, int $maxDepth = 10, int $maxArrayItems = 100)
{
    ...
    if (is_string($value)) {
        if (strlen($value) > 10000) return substr($value, 0, 10000) . '… [truncated]';
        return $value;
    }
...
try {
    fwrite(STDERR, "[ddless] Booting CodeIgniter 4...\n");
    if (class_exists('CodeIgniter\Boot') && class_exists('Config\Paths')) {
        \CodeIgniter\Boot::bootWorker(new \Config\Paths());
    }
    fwrite(STDERR, "[ddless] CodeIgniter 4 booted successfully.\n");
} catch (\Throwable $e) {
    ddless_method_error('Failed to boot CodeIgniter: ' . $e->getMessage(), [
        'file' => $e->getFile(),
        'line' => $e->getLine(),
...
try {
    ...
    if ($isGlobalFunction) {
        ...
        $instance = null;
        if (!$isStatic) {
            // Try CI4 service resolution
            try {
                if (!empty($constructorParameters)) {
                    $instance = $reflection->newInstanceArgs($constructorParameters);
                } elseif (method_exists('\Config\Services', 'getSharedInstance') || is_callable([$targetClass, 'service'])) {
                    // Try to resolve from CI4 service container
                    $shortName = $reflection->getShortName();
                    $serviceName = lcfirst($shortName);
                    if (method_exists('\Config\Services', $serviceName)) {
                        $instance = \Config\Services::$serviceName();
                    }
                }
                ...
                if ($instance === null) {
                    ...
                    if ($constructor === null || $constructor->getNumberOfRequiredParameters() === 0) {
                        ...
                        ddless_method_error('Cannot instantiate class - constructor requires parameters.', [
                            ...
                            'hint' => 'Provide constructor parameters or register the class as a CI4 service.',
                        ]);
                    }
                }
