 ...
 * @license   Proprietary
 *
 * Tempest framework method executor. Boots the Tempest container,
 * resolves services via DI, and executes a single method/function
 * with dependency injection support, returning serialized results
...
if (!defined('DDLESS_PROJECT_ROOT')) {
    // From .ddless/frameworks/tempest/ we need to go up 3 levels to reach project root
...
function ddless_serialize_value(mixed $value, int $depth = 0, int $maxDepth = 10, int $maxArrayItems = 100): mixed
{
    ...
    if (is_object($value)) {
        ...
        // Generic object - extract all accessible properties
        $data = [
            '__class__' => $className,
            '__type__' => 'object',
        ];
...
// Boot Tempest container
$container = null;

try {
    fwrite(STDERR, "[ddless] Booting Tempest framework...\n");
    $container = \Tempest\Core\Tempest::boot(DDLESS_PROJECT_ROOT);
    fwrite(STDERR, "[ddless] Tempest framework booted successfully.\n");
} catch (\Throwable $e) {
    ddless_method_error('Failed to boot Tempest: ' . $e->getMessage(), [
        'file' => $e->getFile(),
        'line' => $e->getLine(),
...
// Evaluate constructor and parameter code
...
try {
    ...
    if ($isGlobalFunction) {
        ...
        if (!$isStatic) {
            try {
                if (!empty($constructorParameters)) {
                    ...
                    // Use Tempest container for autowired resolution
                    $instance = $container->get($targetClass);
                }
            } catch (\Throwable $containerError) {
                // Container failed, try direct instantiation if no required constructor params
                ...
                if ($constructor === null || $constructor->getNumberOfRequiredParameters() === 0) {
                    ...
                    ddless_method_error('Cannot instantiate class - Tempest container failed and constructor requires parameters: ' . $containerError->getMessage(), [
                        'class' => $targetClass,
                        'requiredParams' => $constructor->getNumberOfRequiredParameters(),
                        'hint' => 'Provide constructor parameters or ensure dependencies are registered in the Tempest container.',
                    ]);
                }
            }
        }
        ...
        if ($isStatic) {
            ...
            // Try container->invoke for DI-aware method invocation, fall back to reflection
            try {
                $result = $container->invoke([$instance, $targetMethod], ...$methodParameters);
            } catch (\Throwable $invokeError) {
                // Fallback to direct reflection invocation
