 ...
 * @license   Proprietary
 *
 * Tempest task runner. Boots the Tempest application and executes
 * user-written code with access to models, services, and the container.
 */
...
// Boot Tempest — env is loaded automatically by the framework
try {
    $container = \Tempest\Core\Tempest::boot(DDLESS_PROJECT_ROOT);
} catch (\Throwable $e) {
    ddless_task_emit('error', ['message' => 'Failed to boot Tempest: ' . $e->getMessage()]);
    ddless_task_done(false, $taskStartTime, 'Failed to boot Tempest: ' . $e->getMessage());
    exit(1);
}

// Task Runner Command class
class DdlessTempestTaskRunnerCommand {
    private $__container;
    ...
    // Tempest container helpers
    public function get(string $class) {
        return $this->__container->get($class);
    }
...
// Execute user code
try {
    $command = new DdlessTempestTaskRunnerCommand($container);
    ...
    }, $command, DdlessTempestTaskRunnerCommand::class);
