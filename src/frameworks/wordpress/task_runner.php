 ...
 * @license   Proprietary
 *
 * WordPress task runner. Boots WordPress via wp-load.php so all
 * plugins, themes, hooks, and core functions are available, then
 * executes user-provided PHP code with the DdlessTask output API.
 */
...
// Boot WordPress via wp-load.php — initializes DB, plugins, themes,
// hooks, and all core functions without processing an HTTP request.
$wpLoadPath = DDLESS_PROJECT_ROOT . '/wp-load.php';
if (is_file($wpLoadPath)) {
    ob_start();
    require_once $wpLoadPath;
    ob_end_clean();
    // WordPress booted successfully via wp-load.php
} else {
    fwrite(STDERR, "[ddless] wp-load.php not found, proceeding without WordPress bootstrap\n");
