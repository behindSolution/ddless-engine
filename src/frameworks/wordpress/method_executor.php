 ...
 * @license   Proprietary
 *
 * WordPress method executor. Boots WordPress via wp-load.php so all
 * plugins, themes, and core functions are available, then executes
 * the requested method/function with reflection-based invocation.
 */
...
// Boot WordPress via wp-load.php — this initializes the database,
// loads plugins, themes, and all WordPress functions without
// processing an HTTP request or rendering a template.
$wpLoadPath = DDLESS_PROJECT_ROOT . '/wp-load.php';
if (!is_file($wpLoadPath)) {
    ddless_method_error('wp-load.php not found. Make sure the project root points to the WordPress installation directory.');
}

fwrite(STDERR, "[ddless] Booting WordPress via wp-load.php...\n");
...
if (getenv('DDLESS_DEBUG_MODE') === 'true') {
    ...
    if (file_exists($debugModule)) {
        ...
        if (function_exists('ddless_register_stream_wrapper')) {
            fwrite(STDERR, "[ddless] Registering stream wrapper BEFORE WordPress boot...\n");
...
fwrite(STDERR, "[ddless] WordPress booted successfully.\n");
    ...
    if ($isGlobalFunction) {
        ...
        if (!function_exists($targetMethod)) {
            ddless_method_error("Function not found: {$targetMethod}", [
                'hint' => 'Make sure the function is defined in a plugin or theme that is active.',
            ]);
        }
        ...
        if (!class_exists($targetClass)) {
            ddless_method_error("Class not found: {$targetClass}", [
                'hint' => 'Make sure the class is defined in an active plugin or theme.',
            ]);
        }
