 ...
 * @license   Proprietary
 *
 * Yii 2 task runner. Boots the Yii2 application and executes
 * user-provided PHP code with access to models, components, and services.
 */
...
if (!is_file($configFile)) {
    ddless_task_emit('error', ['message' => 'Yii2 config not found.']);
    ddless_task_done(false, $taskStartTime, 'Yii2 config not found.');
    exit(1);
}
...
try {
    ...
    ddless_task_emit('error', ['message' => 'Failed to boot Yii2: ' . $e->getMessage()]);
    ddless_task_done(false, $taskStartTime, 'Failed to boot Yii2: ' . $e->getMessage());
    exit(1);
}

class DdlessYii2TaskRunnerCommand {
    ...
    public function table($headers, $rows) {
        ...
        ddless_task_emit('table', [
            'headers' => array_values(array_map('strval', (array)$headers)),
            'rows' => $plainRows,
        ]);
    }
    ...
    public function time(string $label, callable $callback) {
        ...
        $result = $callback();
        ddless_task_emit('time', ['label' => $label, 'durationMs' => round((microtime(true) - $start) * 1000, 2)]);
        return $result;
    }
    ...
    public function progress(int $total, string $label = ''): self {
        ...
        ddless_task_emit('progress', [
            'progressId' => $this->__progressId, 'current' => 0,
            'total' => $total, 'percent' => 0, 'label' => $label,
        ]);
        return $this;
    }
    ...
    public function advance(int $step = 1, ?string $label = null): self {
        ...
        ddless_task_emit('progress', [
            'progressId' => $this->__progressId, 'current' => $this->__progressCurrent,
            'total' => $this->__progressTotal, 'percent' => $percent, 'label' => $label,
        ]);
        return $this;
    }

    public function ask($question, $default = null, ?callable $rule = null) {
        $input = ddless_prompt_with_rule('ask', (string)$question, ['promptDefault' => (string)($default ?? '')], $rule);
    ...
    public function confirm($question, $default = false, ?callable $rule = null) {
        $input = ddless_prompt_with_rule('confirm', (string)$question, ['promptDefault' => $default ? 'true' : 'false'], $rule);
        return $input === 'true';
    }

    public function choice($question, $choices, $default = null, $attempts = null, $multiple = false, ?callable $rule = null) {
        if ($choices instanceof \Traversable) { $choices = iterator_to_array($choices); }
        return ddless_prompt_with_rule('choice', (string)$question, [
    ...
    public function anticipate($question, $suggestions, $default = null, ?callable $rule = null) {
        if ($suggestions instanceof \Traversable) { $suggestions = iterator_to_array($suggestions); }
    ...
    public function date($question, ?string $default = null, ?callable $rule = null) {
        return ddless_prompt_with_rule('date', (string)$question, ['promptDefault' => $default ?? date('Y-m-d')], $rule);
    }

    public function datetime($question, ?string $default = null, ?callable $rule = null) {
        return ddless_prompt_with_rule('datetime', (string)$question, ['promptDefault' => $default ?? date('Y-m-d H:i')], $rule);
    }
    ...
    public function import(string $filename, string $delimiter = ',', bool $headers = true, string $encoding = 'auto'): \Generator {
        ...
        try {
            ...
            if ($encoding === 'auto') {
                ...
                if (str_starts_with($sample, "\xEF\xBB\xBF")) { $sample = substr($sample, 3); $encodingDetected = 'UTF-8'; }
                else { $encodingDetected = mb_detect_encoding($sample, ['UTF-8', 'Windows-1252', 'ISO-8859-1'], true) ?: 'UTF-8'; }
                rewind($handle);
                $bom = fread($handle, 3);
            ...
            if ($headers) {
                ...
                if ($firstLine === false || $firstLine === null) { return; }
                if ($needsConversion) { $firstLine = array_map(fn($v) => mb_convert_encoding((string)$v, 'UTF-8', $encodingDetected), $firstLine); }
                $headerRow = $firstLine;
            }
            while (($row = fgetcsv($handle, 0, $delimiter)) !== false) {
                if ($row === null) continue;
                if ($needsConversion) { $row = array_map(fn($v) => mb_convert_encoding((string)$v, 'UTF-8', $encodingDetected), $row); }
                if ($headerRow !== null) { $row = array_pad($row, count($headerRow), ''); yield array_combine($headerRow, array_slice($row, 0, count($headerRow))); }
                else { yield $row; }
            }
    ...
    public function export(string $filename, callable $callback, string $delimiter = ';'): void
    {
        ...
        try {
            while (true) {
                $rows = $callback($page);
                if ($rows === null || $rows === [] || $rows === false) { break; }
                if (!is_array($rows)) { throw new \RuntimeException('Export callback must return an array of rows, null, or an empty array.'); }
                $csvLines = [];
                if (!$headersSent) {
                    $firstRow = reset($rows);
                    if (is_array($firstRow) || is_object($firstRow)) { $csvLines[] = ddless_csv_encode_row(array_keys((array)$firstRow), $delimiter); }
                    $headersSent = true;
                }
            ...
            ddless_task_emit('export_error', ['exportId' => $exportId, 'error' => $e->getMessage(), 'totalRows' => $totalRows]);
            throw $e;
        }
    }
}
...
try {
    $command = new DdlessYii2TaskRunnerCommand();

    $useStatements = '';
    foreach ($imports as $import) {
        $import = trim($import);
        if ($import !== '') { $useStatements .= "use {$import};\n"; }
    }
    ...
    $closure = \Closure::bind(function () use ($__ddless_eval_code__) {
        return eval($__ddless_eval_code__);
    }, $command, DdlessYii2TaskRunnerCommand::class);
